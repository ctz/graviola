...
macro_rules! amontred {
    ($d4:expr, $d3:expr, $d2:expr, $d1:expr, $d0:expr, $t2:expr, $t1:expr, $t0:expr) => { Q!(
        /* We only know the input is -2^316 < x < 2^316. To do traditional  */ /* unsigned Montgomery reduction, start by adding 2^61 * p_256.     */ "mov " $t0 ", #0xe000000000000000;"
        "adds " $d0 ", " $d0 ", " $t0 ";"
        "sbcs " $d1 ", " $d1 ", xzr;"
        ...
        "adcs " $d3 ", " $d3 ", " $t0 ";"
        "adcs " $d4 ", " $d4 ", " $d0 ";"
        /* Now capture top carry and subtract p_256 if set (almost-Montgomery) */ "mov " $t0 ", #0xffffffffffffffff;"
