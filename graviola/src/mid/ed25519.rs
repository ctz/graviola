...
mod test {
    ...
    fn test_rfc8032_test_vectors() {
        ...
        let msg = b"";
        let sig = sk.sign(msg);
        assert_eq!(&low::ct::into_public(sk.verifying_key().as_bytes()), b"\xd7\x5a\x98\x01\x82\xb1\x0a\xb7\xd5\x4b\xfe\xd3\xc9\x64\x07\x3a\x0e\xe1\x72\xf3\xda\xa6\x23\x25\xaf\x02\x1a\x68\xf7\x07\x51\x1a");
        ...
        low::ct::into_public(sk.verifying_key().verify(&sig, msg)).unwrap();
        ...
        let msg = b"\x72";
        let sig = sk.sign(msg);
        assert_eq!(&low::ct::into_public(sk.verifying_key().as_bytes()), b"\x3d\x40\x17\xc3\xe8\x43\x89\x5a\x92\xb7\x0a\xa7\x4d\x1b\x7e\xbc\x9c\x98\x2c\xcf\x2e\xc4\x96\x8c\xc0\xcd\x55\xf1\x2a\xf4\x66\x0c");
